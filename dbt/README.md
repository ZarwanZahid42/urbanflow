...
This directory contains the complete local Phase 8 dbt Core implementation. Seven governed
...
model documentation, and `ref()` lineage are implemented. Offline parsing is complete; no dbt
command has connected to Snowflake yet.
...
2. Select an approved target schema and least-privilege dbt role, then obtain the required
...
4. Copy `profiles.yml.example` to an ignored `dbt/profiles.yml`, or place an equivalent
   `profiles.yml` in the external dbt profiles directory.
5. After access is approved, run `dbt debug`, `dbt build`, and `dbt docs generate` from this
   directory and reconcile the live results with the Phase 7 evidence.
