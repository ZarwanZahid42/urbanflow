...
**Current phase status (2026-08-24):** Phases 1-7 are complete and committed. Phase 7 commit `ea0f67f` contains the live-validated Snowflake integration: all seven Gold datasets load through Databricks Serverless, both idempotency passes are stable, and every final reconciliation gate passes. Phase 8 local implementation is complete with sources, staging, a justified intermediate model, four marts, dbt tests, static contracts, and synchronized documentation. The first approved live Snowflake dbt build and generated catalog remain pending, so Phase 8 is not yet live-validated. ADF and Power BI remain unimplemented.
...
- **Status:** Complete locally and ready for controlled live validation. Seven ANALYTICS sources, seven staging views, one ephemeral intermediate model, four mart views, generic/singular tests, static architecture tests, and repository documentation are implemented. No Snowflake dbt command has run yet.
...
- **Local validation:** Offline `dbt parse` and manifest inspection, the complete Python suite, Python compilation, dependency checks, diff checks, and repository security/artifact scans must pass before review. Generated dbt artifacts are written outside the repository or left ignored.
- **Live completion gate:** After manual access approval, `dbt debug`, `dbt build`, and `dbt docs generate` must succeed against Snowflake; model counts, keys, relationships, and important measures must reconcile with the committed Phase 7 evidence. Live results must then be recorded without committing generated artifacts.
...
- **Manual cloud work:** Select the approved target schema and least-privilege dbt role; grant database and warehouse usage, ANALYTICS read access, and required target-schema relation permissions; confirm approved key-pair authentication; and provide environment values and the private-key path externally. No role name, account identifier, exact grant SQL, or credential is invented. dbt Cloud remains optional and requires explicit selection.
